...
module Riiif
  ...
  class ImagemagickCommandFactory
    ...
    # @return [String] a command for running imagemagick to produce the requested output
    # begin DORIS Override
    def build
      Rails.logger.debug "================ build 1 #{transformation.size}"

      [watermark_pre, external_command, crop, size, rotation, colorspace, quality, sampling, metadata, output, watermark].join
    end

    def watermark_pre
      width = transformation.size.split("x").first.to_i
      @tmpfile = Tempfile.new
      if width < 650
        "convert -resize #{width/2} #{Rails.root.join('app/assets/images/watermark.png')} #{@tmpfile.path} && "
      else
        "cp #{Rails.root.join('app/assets/images/watermark.png')} #{@tmpfile.path} && "
      end

    end

    def watermark
      Rails.logger.debug "================ watermark 1"
      " | composite -watermark 10 -tile #{@tmpfile.path}  - #{transformation.format}:-"
    end
    # end DORIS override
